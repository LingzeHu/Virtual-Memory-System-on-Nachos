...
AddrSpace::AddrSpace(const AddrSpace* add){
    // TranslationEntry* PT = add->getPageTable();
    // numPages = add->getNumPages();
    // // cout << "numPage: " << numPages << endl;

    // pageTable = new TranslationEntry[numPages];
    // for (int i = 0; i < numPages; i++) {
    //     pageTable[i].virtualPage = PT[i].virtualPage;	// for now, virt page # = phys page #
    //     pageTable[i].physicalPage = PT[i].physicalPage;
    //     pageTable[i].valid = PT[i].valid;
    //     pageTable[i].use = PT[i].use;
    //     pageTable[i].dirty = PT[i].dirty;
    //     pageTable[i].readOnly = PT[i].readOnly;  
    // }
...
TranslationEntry* AddrSpace::getPageTable() const{
  return this->pageTable;
}

unsigned int AddrSpace::getNumPages() const{
  return numPages;
}
...
// how big is address space?
    ...
    for (int i = 0; i < numPages; i++) {
        ...
        //write the buffer into swapspace
        kernel->swapSpace->WriteAt(buffer, PageSize, kernel->swapSpace_counter * PageSize );
...
//         // kernel->swapSpace->writeAt(buffer,noffH.code.size, swapSpace_counter);
//         /////////////////////////////============================================
