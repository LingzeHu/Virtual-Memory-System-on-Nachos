...
// For Assign 2
int Thread::getUserRegister(int n){
  if(n > NumTotalRegs)
    return 0;
  return this->userRegisters[n];
}

void Thread::setUserRegister(int n, int content){
  if(n < NumTotalRegs)
    this->userRegisters[n] = content;
}
...
Thread::Sleep (bool finishing)
{
    ...
    if((nextThread = kernel->scheduler->FindNextToRun()) == NULL)
	kernel->interrupt->Halt();	// no one to run, wait for an interrupt
