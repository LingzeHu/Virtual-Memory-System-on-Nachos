...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        programPaths.push_back(userProgName);
        // cout << userProgName << endl;
	    i++;
	}
    ...
    for(int i = 0; i < programPaths.size(); i++ ){
        //cout <<"Forking " <<  programPaths[i] << endl;
        Thread* newThread = new Thread(programPaths[i]);
        newThread->Fork((VoidFunctionPtr)RunUserProg, (char*)programPaths[i]);
    }
